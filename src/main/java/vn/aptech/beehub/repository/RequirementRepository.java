...
public interface RequirementRepository extends JpaRepository<Requirement, Integer> {
	...
	@Query(value = "SELECT r.* FROM requirements r WHERE ((r.sender_id = ?1 AND r.receiver_id = ?2) OR (r.receiver_id = ?1 AND r.sender_id= ?2)) AND r.type = 'ADD_FRIEND' AND r.is_accept=0", nativeQuery = true)
