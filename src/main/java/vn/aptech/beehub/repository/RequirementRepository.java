...
public interface RequirementRepository extends JpaRepository<Requirement, Integer> {
	@Query(value = "SELECT r.* FROM requirements r WHERE r.group_id=?1 AND r.is_accept = 0", nativeQuery = true)
	...
	@Query(value = "SELECT r.* FROM requirements r WHERE ((r.sender_id = ?1 AND r.receiver_id = ?2) OR (r.receiver_id = ?1 AND r.sender_id= ?2)) AND r.type = 'ADD_FRIEND' AND r.is_accept=0 LIMIT 1", nativeQuery = true)
	Optional<Requirement> getRequirementsBtwUsersIsNotAccept(Long user1_id, Long user2_id);
	...
	@Query(value = "SELECT * FROM requirements re WHERE (re.receiver_id = ?1 AND re.is_accept = 0) OR (re.sender_id = ?1 AND re.is_accept = 1)", nativeQuery = true)
	List<Requirement> getNotification(Long id);
}
